...
class ModuleDocFragment:
    ...
    notes:
      - >-
        C(evidence) is the one vocabulary the o0_o namespace names
        provenance in. Every fact a collection in it composes carries
        it, and wherever it appears it is a mapping keyed by kind of
        origin, with the same three kinds. A consumer reads the kind
        off the key it asked for, never off the shape of what it finds
        there.
      - >-
        C(files) names the paths that were read. Where the collection
        publishes a store of paths, as M(o0_o.posix.facts) does in
        C(o0_paths), each is a key of it, so an entry joins against the
        file it came out of rather than being told that some file
        exists somewhere.
      - >-
        C(commands) names the commands that were consulted, by name
        and not by argv. Argv would say what was typed, and what was
        typed is a debugging concern rather than a fact - a
      ...
      - >-
        ...
        in the namespace holds to; a kind absent altogether says it is
        not one this producer has. C(o0_users) reads files and runs
        commands and is composed from no configuration variable, so
        its entries carry two kinds and not the third. Each list is
        sorted and holds one of each name.
      - >-
        A producer whose source is the Ansible process itself - the
        connection the executor built for a task, the credentials the
        controller runs under - attempts none of the three kinds, and
        says so with an empty C(evidence) rather than by inventing a
        fourth. The record is present because the statement is one:
        nothing outside Ansible was consulted, and the fact is exactly
        what the process holds. C(o0_connection) is such a fact, and
        so is C(o0_controller) until its config subset reads a file.
      - >-
        ...
        C(o0_storage), C(o0_connection), C(o0_controller) - the
        section carries one record rather than every entry under it
        repeating the same answer. Several subsets may answer for one
        section, and the section's evidence is the union of what each
        of them consulted.
      - >-
      ...
      - >-
        ...
        asks with instead: M(o0_o.posix.read)'s permission probe names
        C(test), and C(su) where it drops identity; its resolution
        walk names C(cd), C(pwd) and C(ls). Where the shell is itself
        the subject, the interpreter is the answer and stays - the
      ...
      - >-
        ...
        contributing to a fact another one publishes names itself
        there rather than inventing a field of its own.
      - >-
        ...
        own answer; and C(o0_os), C(o0_network), C(o0_hardware),
        C(o0_storage), C(o0_connection) and C(o0_controller) carry one
        for the section, because one gather produced it. A mapping
        that does not say what it consulted claims no producer; a
        mapping that says it consulted nothing still names the
        producer that said so.
      - >-
