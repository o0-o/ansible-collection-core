...
"""
Cross-platform whoami action plugin using COMMAND_SPEC pattern.

Demonstrates the command specification approach for cross-platform
command execution with templating and parsing.
"""
...
class ActionModule(CoreActionBase, ActionBase):
    """Get the current user via the whoami command."""
    ...
    def run(
        ...
        """Execute whoami command and return the current user.

        :param Optional[str] tmp: Unused temporary directory path
        :param Optional[dict[str, Any]] task_vars: Available Ansible
            variables
        :returns dict[str, Any]: Result dictionary with 'user' key
        """
        ...
        if not cmd_requests:
            result["failed"] = True
            result["msg"] = "No command specification found for 'whoami'"
            return result
