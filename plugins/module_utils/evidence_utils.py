...
Every fact the o0_o namespace composes says where it came from under
...
read, each a key of the path store where the collection publishes one
(``o0_paths`` in o0_o.posix).  ``commands`` holds the names of the
...
and what was typed is a debugging concern rather than a fact: a
...
The vocabulary lives here rather than in the collection that first
spoke it because the ladder runs utils, core, posix, and a fact this
collection publishes about the controller or the connection has to
name its provenance in the same words a posix fact does.  A producer
whose source is the Ansible process itself - the play context, its own
credentials - consulted nothing of the three kinds and says so with an
empty record, which still names its producer.
"""
