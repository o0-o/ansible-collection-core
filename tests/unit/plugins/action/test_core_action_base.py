...
class TestGetCommandErrorPrefix:
    ...
    def test_valid_prefix(
        ...
        assert base._get_command_error_prefix(cmd_obj) == expected

    def test_missing_implementation(self, base) -> None:
    ...
    def test_empty_type(self, base) -> None:
...
class TestProcessCommandResult:
    """Tests for _process_command_result method."""

    def test_successful_command_no_parser(self, base) -> None:
        """Test processing successful command without parser."""
        cmd_completed = {
            "implementation": "core",
            "type": "whoami",
            "result": {"rc": 0, "stdout": "testuser\n", "stderr": ""},
        }
        output, errors = base._process_command_result(cmd_completed)
        assert output == "testuser"
        assert errors is None

    def test_successful_command_strips_newlines(self, base) -> None:
        """Test that trailing newlines are stripped from output."""
        cmd_completed = {
            "implementation": "core",
            "type": "test",
            "result": {"rc": 0, "stdout": "output\n\n", "stderr": ""},
        }
        output, errors = base._process_command_result(cmd_completed)
        assert output == "output"

    def test_successful_command_strips_carriage_returns(self, base) -> None:
        """Test that carriage returns are stripped from output."""
        cmd_completed = {
            "implementation": "core",
            "type": "test",
            "result": {"rc": 0, "stdout": "line1\r\nline2\r\n", "stderr": ""},
        }
        output, errors = base._process_command_result(cmd_completed)
        assert output == "line1\nline2"

    def test_failed_command_nonzero_rc(self, base) -> None:
        """Test processing command with non-zero return code."""
        cmd_completed = {
            "implementation": "core",
            "type": "test",
            "result": {"rc": 1, "stdout": "", "stderr": "error message"},
        }
        output, errors = base._process_command_result(cmd_completed)
    ...
    def test_failed_command_no_stderr(self, base) -> None:
        """Test processing failed command with no stderr."""
        cmd_completed = {
            "implementation": "core",
            "type": "test",
            "result": {"rc": 1, "stdout": "", "stderr": ""},
        }
        output, errors = base._process_command_result(cmd_completed)
        assert output is None
        assert "No stderr" in str(errors[0])

    def test_custom_non_error_codes(self, base) -> None:
        """Test processing with custom non-error return codes."""
        cmd_completed = {
            "implementation": "core",
            "type": "test",
            "result": {"rc": 1, "stdout": "output", "stderr": ""},
        }
        output, errors = base._process_command_result(
            cmd_completed, non_error_codes=[0, 1]
        )
        assert output == "output"
        assert errors is None

    def test_with_validator_success(self, base) -> None:
        """Test processing with validator that passes."""
        cmd_completed = {
            "implementation": "core",
            "type": "hello_world",
            "result": {"rc": 0, "stdout": "Hello, world!", "stderr": ""},
    ...
    def test_with_validator_failure(self, base) -> None:
    ...
    def test_with_parser_success(self, base) -> None:
        ...
        cmd_completed = {
            "implementation": "core",
            "type": "test",
            "result": {"rc": 0, "stdout": "hello", "stderr": ""},
            "parser": parser,
        }
        output, errors = base._process_command_result(cmd_completed)
        assert output == "HELLO"
    ...
    def test_with_parser_failure(self, base) -> None:
        ...
        cmd_completed = {
            "implementation": "core",
            "type": "test",
            "result": {"rc": 0, "stdout": "data", "stderr": ""},
            "parser": parser,
        }
        output, errors = base._process_command_result(cmd_completed)
        assert output is None
    ...
    def test_missing_result(self, base) -> None:
        """Test TypeError raised when result is missing."""
        cmd_completed = {
            "implementation": "core",
            "type": "test",
        }
        with pytest.raises(TypeError, match="not a dict"):
            base._process_command_result(cmd_completed)

    def test_missing_rc(self, base) -> None:
    ...
    def test_missing_stdout(self, base) -> None:
    ...
    def test_non_callable_parser(self, base) -> None:
    ...
    def test_non_callable_validator(self, base) -> None:
...
class TestProcessCommandSpec:
    """Tests for _process_command_spec method."""

    def test_whoami_spec(self, base) -> None:
        """Test processing whoami command spec."""
        requests = base._process_command_spec("whoami")
        assert len(requests) == 1
        request = requests[0]
        assert request["implementation"] == "core"
        assert request["type"] == "whoami"
        assert request["command"] == ("whoami",)
        assert request["name"] == "whoami"

    def test_hello_world_spec(self, base) -> None:
        """Test processing hello_world command spec."""
        requests = base._process_command_spec("hello_world")
        assert len(requests) == 1
        request = requests[0]
        assert request["implementation"] == "core"
    ...
    def test_nonexistent_command(self, base) -> None:
        """Test processing nonexistent command returns empty list."""
        requests = base._process_command_spec("nonexistent")
        assert requests == []

    def test_template_formatting(self, base) -> None:
        """Test that templates are properly formatted with kwargs."""
        # Create a custom spec with placeholder
        original_spec = base.COMMAND_SPEC.copy()
        base.COMMAND_SPEC = {
            "test": {
                "greet": {
        ...
        try:
            requests = base._process_command_spec("greet", name="Alice")
            assert len(requests) == 1
            assert requests[0]["command"] == ("echo", "Hello, Alice!")
        finally:
            base.COMMAND_SPEC = original_spec

    def test_string_template(self, base) -> None:
        ...
        try:
            requests = base._process_command_spec("cmd")
            assert len(requests) == 1
            assert requests[0]["command"] == "echo hello"
        finally:
            base.COMMAND_SPEC = original_spec

    def test_missing_template(self, base) -> None:
        ...
        try:
            with pytest.raises(ValueError, match="missing a template"):
                base._process_command_spec("cmd")
        finally:
            base.COMMAND_SPEC = original_spec

    def test_empty_template(self, base) -> None:
