...
ansible-galaxy collection install /build/o0_o-core-*.tar.gz --force -p /root/.ansible/collections -vvv

# Navigate to installed collection for running tests
cd /root/.ansible/collections/ansible_collections/o0_o/core
...
su testuser -c "
	...
	ansible --version
	# Install collection from built tarball with dependencies
	ansible-galaxy collection install /build/o0_o-core-*.tar.gz --force -p ~/.ansible/collections -vvv
	# Navigate to installed collection
	cd ~/.ansible/collections/ansible_collections/o0_o/core
	${testuser_cmd}
"
